 ...
 *
 */
// xem thử trong area có còn cái nào free không, nếu free thì cho rg_elmt vào
 ...
 *
 */
// cấp phát bộ nhớ cho region
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  /*Allocate at the toproof */
  if(caller->mm->symrgtbl[rgid].rg_start<caller->mm->symrgtbl[rgid].rg_end) pgfree_data(caller, rgid);
 ...
 *
 */
// hàm này nó lấy không gian bộ nhớ ảo ánh xạ vào bộ nhớ vật lý
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller) {
    uint32_t pte = mm->pgd[pgn];
    if (PAGING_PAGE_SWAPPED(pte)) { /* Page is not online, make it actively living */
        // swpfpn số frame trống trong ổ đĩa
        int vicpgn, swpfpn;    
        int vicfpn;
        // pte của victom
        uint32_t vicpte;
        ...
        int tgtswptp = PAGING_SWAPTYPE(pte);
        ...
        if (MEMPHY_get_freefp(caller->active_mswp, &swpfpn) < 0) {
            ...
            for (i = 0; i < PAGING_MAX_MMSWP; i++) {
                struct memphy_struct *tmp_swp = *mswpit;
        ...
        //__swap_cp_page();
        __swap_cp_page(mswp + tgtswptp, tgtfpn, caller->mram, vicfpn);
        /* Update page table */
        //pte_set_swap() &mm->pgd;
        MEMPHY_put_freefp(mswp + tgtswptp, tgtfpn);
 ...
 *
 */
// tạo ra một địa chỉ mới có kích thước size đằng sau của sbrk
 ...
 *
 */
// hàm kiểm tra xem thử phần area trên hoặc dưới còn chỗ hay không có bị đè lên hay không
 ...
 *
 */
// hàm kiểm tra thử thêm region có kích thước inc_sz vào thì được không
